  ...
  const setApproval = async (amount) => {
    try {
      const approval = await writeContractAsync({
        ...
        args: [
          blockConfig[chainId].XRADE_ADDRESS,
          amount,
        ],
      });
      ...
      {packages.map((pkg, index) => (
        ...
        >
          ...
          <button className="bg-blue text-white py-2 px-4 rounded-full hover:bg-[#192265] hover:shadow-buypackage"
            onClick={() => handleBuyPackage(index, referrer)} // Pass null for referrer if not available
            disabled={isTransaction}
          >
            {isTransaction ? transactionMessage : "Buy Package"}
          </button>
        </div>
      ))}
    </div>
